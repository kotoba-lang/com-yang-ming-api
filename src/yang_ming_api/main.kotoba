(ns yang-ming-api.main
  "Kotodama WASM entrypoint for the Yang Ming API clean-room actor (L4) — Clojure port.
  ...
  facts (`yang_ming_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "yang_ming_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "yangming_tra"
    :fields [:gtin :description :brand]        :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"       :id-prefix "yangming_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents"    :id-prefix "yangming_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers"   :id-prefix "yangming_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"      :id-prefix "yangming_par"
    :fields [:gln :name :role]                 :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments"    :id-prefix "yangming_shi"
    :fields [:documentId :origin :destination :status] :required [:origin :destination]
...
(defn emit-facts
  "EAVT facts for one record: {\"yang_ming_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yang_ming_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yang_ming_api-compat/ready)
