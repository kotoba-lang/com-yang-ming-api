(ns yang-ming-api.main-test
  "Contract + behavioral test for the yang-ming-api L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "yang_ming_api-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'yang-ming-api.main-test)]
